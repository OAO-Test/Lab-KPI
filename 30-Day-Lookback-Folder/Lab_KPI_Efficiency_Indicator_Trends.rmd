...
cp_dashboard_tat_summary <- cp_hist %>%
  ...
  mutate(ReceiveResultDefects = ResultedVolumeWithTAT - ReceiveResultInTarget,
         ...
         SafeThreshold = ifelse(Test == "Rapid Flu" |
                                  Test == "C. diff", 1.0, 0.95),
         NotSafeThreshold = ifelse(Test == "Rapid Flu" |
                                     Test == "C. diff", 0.9, 0.8))
...
cp_dashboard_defect_summary <- cp_dashboard_tat_filter %>%
  ...
  summarize(ResultedVolume = sum(ResultedVolume),
            ...
            ReceiveResultDefects =
              ResultedVolumeWithTAT - ReceiveResultInTarget,
            ReceiveResultDefectRate =
              ReceiveResultDefects / ResultedVolumeWithTAT)
  ...
  function(data_defect, test_name, site_name, lab_priority) {
    ...
    center_line <- 
      mean(QC_Lines(data = data_defect_plot$ReceiveResultDefectRate,
                    n = data_defect_plot$ResultedVolumeWithTAT,
                    method = "p")$pBar)
    #
    # Create plot and standard settings
...
cp_stratified_data_for_pcharts <- function(defect_data, test) {
  ...
  for (site in sites) {
    ...
    for (priority in priorities) {
      ...
      avg_resulted_volume <- 
        mean(priority_df$ResultedVolumeWithTAT[
