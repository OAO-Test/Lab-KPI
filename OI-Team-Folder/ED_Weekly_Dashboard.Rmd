...
resulted_volume_kable <- function (site, test) {
  ...
  kable(resulted_lab_volume, format = "html", escape = FALSE, align = "c",
        caption = paste(site, test, "Resulted Volume by Week")) %>%
...
# Custom function for creating a table with missing collections
missing_collect <- cp_hist %>%
  filter(Site == "MSH" &
           Test == "Troponin" &
           WeekStart >= this_week_start - 12 * 7) %>%
  group_by(WeekStart, MasterSetting) %>%
  rename(Setting = MasterSetting) %>%
  summarize(TotalMissingCollections = sum(TotalMissingCollections, na.rm = TRUE),
  ...
  mutate(WeekStart = ifelse(WeekStart == "Total",
                            lag(WeekStart, 1),
                            WeekStart),
         WeekStart = format(as.Date(WeekStart, format = "%Y-%m-%d"), "%m/%d/%y"),
...
kable(missing_collect, format = "html", escape = FALSE, align = "c",
      caption = paste("MSH", "Troponin", "Missing Collections Summary")) %>%
  kable_styling(bootstrap_options = "hover", position = "center",
                  font_size = 11) %>%
    column_spec(column = c(1),
                border_right = "thin solid lightgray") %>%
    add_header_above(c(" " = 1,
                       "Week Of" = ncol(missing_collect) - 1),
                     background = c("white", "#221f72"),
                     color = "white", line = FALSE, font_size = 13) %>%
    row_spec(row = 0, font_size = 13) %>%
    row_spec(row = (nrow(missing_collect) - 1):nrow(missing_collect), bold = TRUE, hline_after = TRUE) %>%
  collapse_rows()
