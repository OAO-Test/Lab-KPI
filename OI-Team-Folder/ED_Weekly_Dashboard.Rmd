...
# Custom function for creating a table with missing collections across all settings
missing_collection_kable_all_settings <- function(site, test) {
  # Create a dataframe with missing collections for this site and test over the last 3 months
  missing_collect_summary <- cp_hist %>%
    filter(Site == site &
             Test == test &
             WeekStart >= this_week_start - lookback_period * 7) %>%
    ...
    summarize(TotalMissingCollections = sum(TotalMissingCollections,
              ...
              .groups = "keep") %>%
    arrange(WeekStart) %>%
    # Split dataframe into a list of dataframes by each week to calculate weekly total
    split(.[,"WeekStart"]) %>%
    # Calculate weekly total for each week and then combine list of dataframes
    # into 1 dataframe
    map_df(., adorn_totals, fill = "Total") %>%
    # Rename "total" with the appropriate week start date
    mutate(WeekStart = ifelse(WeekStart == "Total",
                              lag(WeekStart, 1),
                              WeekStart),
           # Convert week start to date and then back to text for better
           # table formatting
           WeekStart = format(as.Date(WeekStart, format = "%Y-%m-%d"),
                              "%m/%d/%y"),
           # Calculate percent of missing labs
           PercentMissing = percent(TotalMissingCollections /
                                    TotalResultedVolume,
           ...
           # longer with percent missing
           TotalMissingCollections = as.character(TotalMissingCollections)) %>%
    select(WeekStart, Setting, TotalMissingCollections, PercentMissing) %>%
    # Pivot longer to get volume and percentage of missing labs in 1 column
    pivot_longer(cols = c(TotalMissingCollections,
                          PercentMissing),
                 names_to = "Metric") %>%
    ...
    pivot_wider(names_from = WeekStart,
                names_sort = FALSE) %>%
    mutate(Metric = ifelse(Metric == "TotalMissingCollections",
                           "Vol of Labs", "% of Labs"),
           Setting = factor(Setting, levels = pt_setting, ordered = TRUE)) %>%
    arrange(Setting) %>%
  ungroup()
  
  kable(missing_collect_summary, format = "html", escape = FALSE, align = "c",
    ...
    add_header_above(c(" " = 2,
                     ...
                     color = "white", line = FALSE, font_size = 13) %>%
    row_spec(row = 0, font_size = 13) %>%
    row_spec(row = (nrow(missing_collect_summary) - 1):
               nrow(missing_collect_summary),
             bold = TRUE,
             hline_after = TRUE) %>%
    collapse_rows()
}
...
missing_collection_kable_all_settings(site = "MSB", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSBI", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSH", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSM", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSQ", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSSN", test = "Troponin")
...
missing_collection_kable_all_settings(site = "MSW", test = "Troponin")
