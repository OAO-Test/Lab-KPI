...
cp_user_path <- paste0(user_directory, "/SCC Sunquest Historical Repo/*.*")
...
* Receive to Result TAT: for anatomic pathology the receive to result TAT; lab KPI metric, is calculated by only using the MSHS business days.

* Collect to Result TAT: for anatomic pathology the collect to result TAT; patient KPI metric, is calculated using all the calendar days.

* Primary Specimens: when any specimen has multiple slides only the first slide is included (spec_sort_order = A).
...
* All Breast Specimens:
    ...
    + TAT analysis excludes labs with missing timestamps (none valid).
