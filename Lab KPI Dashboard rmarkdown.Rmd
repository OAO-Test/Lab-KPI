...
#import table template
Table_Template_2 <- read_excel(choose.files(caption = "Select Table Dataframe"), sheet = "Patho") 
names(Table_Template_2) <- c("spec_group","Patient.Setting","No_Cases_Signed","MSH.x", "BIMC.x","MSQ.x", "MSS.x","NYEE.x","PACC.x","R.x","SL.x", "KH.x","BIMC.y", "MSH.y", "MSQ.y", "MSS.y","NYEE.y","PACC.y","R.y","SL.y", "KH.y")

Pathology_Table_Weekday_New <- merge(x = Table_Template_2, y= Pathology_Table_Weekday, all.y=TRUE)

#second step is merging the table with all of the columns with only the first two columns of the template to include all the missing rows
Pathology_Table_Weekday_New2 <- merge(x = Table_Template_2[c(1,2)], y= Pathology_Table_Weekday_New, all.x = TRUE, by = c("spec_group", "Patient.Setting"))
...
if (is.null(PP_Not_Weekday)|nrow(Surgical_Pathology_Not_Weekday==0)){
  ...
  #first step is merging the table template with the cytology table and this will include all of the missing columns.
  Pathology_Table_Not_Weekday_New <- merge(x = Table_Template_2, y= Pathology_Table_Not_Weekday, all.y=TRUE)
  
  #second step is merging the table with all of the columns with only the first two columns of the template to include all the missing rows
  Pathology_Table_Not_Weekday_New2 <- merge(x = Table_Template_2[c(1,2)], y= Pathology_Table_Not_Weekday_New, all.x = TRUE, by = c("spec_group", "Patient.Setting"))
...
Cytology_Table_Weekday_New2 %>%
  mutate(
    MSH.x = ifelse(MSH.x > 0.9, cell_spec(MSH.x, "html", background  = "lightgreen"),
                   ifelse(MSH.x < 0.8, cell_spec(MSH.x, "html", background = "red"),
                          ifelse(is.na(MSH.x), cell_spec(MSH.x, "html", color = "grey"), cell_spec(MSH.x, "html", background = "yellow")))),
    
    BIMC.x = ifelse(BIMC.x > 0.9, cell_spec(BIMC.x, "html", background  = "lightgreen"),
                   ifelse(BIMC.x < 0.8, cell_spec(BIMC.x, "html", background = "red"),
                          ifelse(is.na(BIMC.x), cell_spec(BIMC.x, "html", color = "grey"), cell_spec(BIMC.x, "html", background = "yellow")))),
    MSQ.x = ifelse(MSQ.x > 0.9, cell_spec(MSQ.x, "html", background = "lightgreen"),
                   ifelse(MSQ.x < 0.8, cell_spec(MSQ.x, "html", background = "red"),
                          ifelse(is.na(MSQ.x), cell_spec(MSQ.x, "html", color = "grey"), cell_spec(MSQ.x, "html", background = "yellow")))),
    MSS.x = ifelse(MSS.x > 0.9, cell_spec(MSS.x, "html", background = "lightgreen"),
                   ifelse(MSS.x < 0.8, cell_spec(MSS.x, "html", background= "red"),
                          ifelse(is.na(MSS.x), cell_spec(MSS.x, "html", color = "grey"), cell_spec(MSS.x, "html", background = "yellow")))),
    NYEE.x = ifelse(NYEE.x > 0.9, cell_spec(NYEE.x, "html", background = "lightgreen"),
                   ifelse(NYEE.x < 0.8, cell_spec(NYEE.x, "html", background = "red"),
                          ifelse(is.na(NYEE.x), cell_spec(NYEE.x, "html", color = "grey"), cell_spec(NYEE.x, "html", background = "yellow")))),
    PACC.x = ifelse(PACC.x > 0.9, cell_spec(PACC.x, "html", background = "lightgreen"),
                   ifelse(PACC.x < 0.8, cell_spec(PACC.x, "html", background = "red"),
                          ifelse(is.na(PACC.x), cell_spec(PACC.x, "html", color = "grey"), cell_spec(PACC.x, "html", background = "yellow")))),
    R.x = ifelse(R.x > 0.9, cell_spec(R.x, "html", background = "lightgreen"),
                   ifelse(R.x < 0.8, cell_spec(R.x, "html", background = "red"),
                          ifelse(is.na(R.x), cell_spec(R.x, "html", color = "grey"), cell_spec(R.x, "html", background = "yellow")))),
    SL.x = ifelse(SL.x > 0.9, cell_spec(SL.x, "html", background = "lightgreen"),
                   ifelse(SL.x < 0.8, cell_spec(SL.x, "html", background = "red"),
                          ifelse(is.na(SL.x), cell_spec(SL.x, "html", color = "grey"), cell_spec(SL.x, "html", background = "yellow")))),
    KH.x = ifelse(KH.x > 0.9, cell_spec(KH.x, "html", background = "lightgreen"),
                   ifelse(KH.x < 0.8, cell_spec(KH.x, "html", background = "red"),
                          ifelse(is.na(KH.x), cell_spec(KH.x, "html", color = "grey"), cell_spec(KH.x, "html", background = "yellow"))))
    ) %>%
  select(everything()) %>%
  kable(escape = F) %>%
...
background_color <- function(x){
  ...
  else if (x >= 0.9){return("green")}
  else if (x <= 0.8){return("red")}
  else {return("yellow")}
}
Cytology_Table_Weekday_New2 <- as.data.frame(Cytology_Table_Weekday_New2)

formattable(Cytology_Table_Weekday_New2, align = "c", list(area(col = 4:12) ~ formatter("span", style = x ~ style(color=sapply(x,background_color)))))
