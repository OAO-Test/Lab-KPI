...
#To add all the missing rows and columns to the data
...
Cytology_Table_Weekday %>%
  ...
  add_header_above(c(" "= 3, "Average Collection to Result TAT (Calendar Days)"= 8, "Receive to Result TAT "=8), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
  column_spec(4:11, background = "#00B9F2", color= "white",include_thead = TRUE)%>%
  row_spec(1:4, background = "white", color = "black")%>%
  column_spec(12:19, background = "#221F72",color="white", include_thead = TRUE)%>%
  row_spec(1:4, background = "white", color="black")%>%
  collapse_rows(columns = 1)%>%
  add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=19))%>%
  add_header_above(c("Cytology Efficiency Indicator"=19))
```
