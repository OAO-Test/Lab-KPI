...
Cytology_Table_Weekday %>%
  ...
  add_header_above(c(" "= 3, "Average Collection to Result TAT (Calendar Days)"= 8, "Receive to Result TAT "=8))%>%
  column_spec(4:11, background = "#D80B8C", color= "white",include_thead = TRUE)%>%
...
Pathology_Table_Weekday %>%
  kable() %>%
  kable_styling(c("hover","condensed","responsive","bordered"), full_width = FALSE, position = "left", font_size = 12) %>%
  add_header_above(c(" "= 3, "Average Collection to Result TAT (Calendar Days)"= 9, "Receive to Result TAT "=9))%>%
  column_spec(4:12, background = "#D80B8C", color= "white",include_thead = TRUE)%>%
  row_spec(1:7, background = "white", color = "black")%>%
