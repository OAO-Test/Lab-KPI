...
Conditional_Formatting <- function(Table_New2){
  if (is.null(Table_New2)){
    ...
    Table_New3 <- Table_New3 %>%
      mutate(value = ifelse(value > 0.9, cell_spec(value, "html", color  = "green"),
                       ifelse(value < 0.8, cell_spec(value, "html", color = "red"),
                              ifelse(is.na(value), cell_spec(value, "html", color = "light gray"), cell_spec(value, "html", color = "orange")))))
    ...
    Table_New3$Received.to.signed.out.target..Days.<- paste("<= ", Table_New3$Received.to.signed.out.target..Days., "mins")
...
Cytology_Table_Weekday_New3 %>%
  ...
  column_spec(1:4, color = "grey", include_thead = TRUE)%>%
  ...
  add_header_above(c("Cytology Efficiency Indicator (Weekday)"=22))
...
if (is.null(Cytology_Table_Not_Weekday_New3)){
  ...
  add_header_above(c(" "= 4, "Receive to Result TAT within Target"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
  add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=22))%>%
  add_header_above(c("Cytology Efficiency Indicator (Weekend and/or Holiday)"=22))
}
...
Surgical_Pathology_Table_Weekday_New3 %>%
  ...
  add_header_above(c(" "= 4, "Receive to Result TAT within Target"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
  add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=22))%>%
  add_header_above(c("Anatomic Pathology Efficiency Indicator (Weekday)"=22))

if (is.null(Surgical_Pathology_Table_Not_Weekday_New3)){
  Surgical_Pathology_Table_Not_Weekday_New3 <- NULL
} else{
  Surgical_Pathology_Table_Not_Weekday_New3 %>%
  select(everything()) %>%
  kable(escape = F, align = "c", col.names = c("Case Type","Target","Setting","No. Cases Signed","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE")) %>%
  kable_styling(c("hover","condensed","responsive"), full_width = FALSE, position = "center", row_label_position = "c", font_size = 11) %>%
  column_spec(5:13, background = "#00B9F2",include_thead = TRUE)%>%
  ...
  add_header_above(c(" "= 4, "Receive to Result TAT within Target"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
  add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=22))%>%
  add_header_above(c("Anatomic Pathology Efficiency Indicator (Weekend and/or Holiday)"=22))
}
```
