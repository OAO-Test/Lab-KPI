...
Table_Merging_Patho <- function(Surgical_Pathology_Table){
  if (is.null(Surgical_Pathology_Table)){
    ...
    Surgical_Pathology_Table_New2 <- Surgical_Pathology_Table_New2[order(rows_order), columns_order]
  }
...
Cytology_Table_Weekday_New3 %>%
  select(everything()) %>%
  kable(escape = F, align = "c", col.names = c("Case Type","Setting","No. Cases Signed","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE")) %>%
...
Surgical_Pathology_Table_Weekday_New3 %>%
  ...
  add_header_above(c(" "= 3, "Receive to Result TAT within Target"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
    add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=21))%>%
    add_header_above(c("Anatomic Pathology Efficiency Indicator (Weekday)"=21))
...
if (is.null(Surgical_Pathology_Table_Not_Weekday_New3)){
  ...
  add_header_above(c(" "= 3, "Receive to Result TAT within Target"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")%>%
    add_header_above(c("Status Definitions: Green >= 90%, Yellow >= 80% & < 90%, Red < 80%"=21))%>%
    add_header_above(c("Anatomic Pathology Efficiency Indicator (Weekend or/and Holiday)"=21))
}
```
