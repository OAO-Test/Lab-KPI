...
```{r Data Preprocessing and Merging, echo = FALSE}
...
chemistry[c("ReceiveResultTarget", "CollectResultTarget")] <- lapply(chemistry[c("ReceiveResultTarget", "CollectResultTarget")], function(x) paste0("<=", x, " min"))
...
chemistry_dashboard$value <- percent(chemistry_dashboard$value, digits = 0)
...
chem_table %>%
  ...
  add_header_above(c(" " = 1, "Receive to Result Within Target" = (ncol(chem_table)-1)/2, "Collect to Result Within Target" = (ncol(chem_table)-1)/2), background = c("white", "#00AEEF", "#221f72"), color = "white") %>%
  column_spec(column = c(1, as.integer(ncol(chem_table))/2 + 1, ncol(chem_table)), border_right = "thin solid lightgray") %>%
  ...
  column_spec(column = 1, width_min = "10em") %>%
