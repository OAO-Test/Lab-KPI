...
preprocess_scc_sun <- function(raw_scc, raw_sun)  {
  ...
  raw_scc$TestPriority <- paste(raw_scc$Test, "-", raw_scc$DashboardPriority, "Labs")
  ...
  scc_master <- raw_scc[ , c("Ward", "WARD_NAME", "WardandName", 
                                   ...
                                   "AdjPriority", "DashboardPriority", "TestPriority",
  ...
  colnames(scc_master) <- c("LocCode", "LocName", "LocConcat", 
                                 ...
                                 "Site", "ICU", "LocType", 
                                 "Setting", "SettingRollUp", "MasterSetting", "DashboardSetting",
                                 "AdjPriority", "DashboardPriority", "Test-Priority", 
  ...
  raw_sun$TestPriority <- paste(raw_sun$Test, "-", raw_sun$DashboardPriority, "Labs")
...
chemistry <- scc_sun_wday_master[scc_sun_wday_master$Division == "Chemistry" & scc_sun_wday_master$TATInclude == TRUE, ] %>%
  group_by(Test, Site, DashboardPriority, DashboardSetting, ReceiveResultTarget, CollectResultTarget) %>%
...
chemistry[c("ReceiveResultTarget", "CollectResultTarget")] <- lapply(chemistry[c("ReceiveResultTarget", "CollectResultTarget")], function(x) paste("<=", x, "min"))

chemistry_dashboard <- melt(chemistry, id.var = c("Test", "Site", "DashboardPriority", "DashboardSetting", "ReceiveResultTarget", "CollectResultTarget"), measure.vars = c("ReceiveResultPercent", "CollectResultPercent"))
...
chemistry_summary <- dcast(chemistry_dashboard_2, Test + DashboardPriority + paste(Test, "-", DashboardPriority, "Labs") + DashboardSetting +ReceiveResultTarget + CollectResultTarget ~ variable + Site, value.var = "value")

chemistry_summary <- chemistry_summary[ , c(1:2, 4, 3, 6:11, 5, 3, 12:17)]

chemistry_summary[ , 3:ncol(chemistry_summary)] %>%
  ...
  add_header_above(c(" " = 2, "Receive to Result Within Target" = 8, "Collect to Result Within Target" = 8)) %>%
  column_spec(column = c(2, 10), border_right = TRUE) %>%
  collapse_rows(columns = c(1, 2, 3, 11), row_group_label_position = "identity") 
