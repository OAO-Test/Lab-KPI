...
Yest <- as.Date("11/28/2019", format = "%m/%d/%Y")
...
if (is.null(SQ_Not_Weekday) & is.null(SQ_Not_Weekday)) {
  include_not_wkday <- FALSE
  scc_sun_not_wkday_list <- NULL
  scc_not_wkday <- NULL
  sun_not_wkday <- NULL
  scc_sun_not_wkday_master <- NULL
} else {
  include_not_wkday <- TRUE
  scc_sun_not_wday_list <- preprocess_scc_sun(SCC_Not_Weekday, SQ_Not_Weekday)
  scc_not_wkday <- scc_sun_not_wday_list[[1]]
  sun_not_wkday <- scc_sun_not_wday_list[[2]]
  scc_sun_not_wkday_master <- scc_sun_not_wday_list[[3]]
}
```

```{r Manipulate and Reshape Data for Dashboards, warning = FALSE, message = FALSE, echo = FALSE}
# Manipulate and reshape SCC and Sunquest data -----------------------------------------------
# CHEMISTRY: Begin by subsetting data to be included in TAT analysis
chem_sub <- scc_sun_wday_master[scc_sun_wday_master$Division == "Chemistry" & scc_sun_wday_master$TATInclude == TRUE, ]

# Update factors for Chemistry data to remove unnecessary combinations
chem_sub$Test <- factor(chem_sub$Test, unique(test_code$Test[test_code$Division == "Chemistry"]))
chem_sub$DashboardSetting <- factor(chem_sub$DashboardSetting, dashboard_pt_setting)
chem_sub$DashboardPriority <- droplevels(chem_sub$DashboardPriority)
...
# HEMATOLOGY: Begin by subsetting data to be included in TAT analysis
hem_sub <- scc_sun_wday_master[scc_sun_wday_master$Division == "Hematology" & scc_sun_wday_master$TATInclude == TRUE, ]

# Update factors for Micro data to remove unnecessary combinations
hem_sub$Test <- factor(hem_sub$Test, unique(test_code$Test[test_code$Division == "Hematology"]))
hem_sub$DashboardSetting <- factor(hem_sub$DashboardSetting, dashboard_pt_setting)
hem_sub$DashboardPriority <- droplevels(hem_sub$DashboardPriority)
...
# MICRO RRL: Begin by subsetting Micro data to be included in TAT analysis
micro_sub <- scc_sun_wday_master[scc_sun_wday_master$Division == "Microbiology RRL" & scc_sun_wday_master$TATInclude == TRUE, ]

# Update factors for Micro data to remove unnecessary combinations
micro_sub$Test <- factor(micro_sub$Test, unique(test_code$Test[test_code$Division == "Microbiology RRL"]))
micro_sub$DashboardSetting <- factor(micro_sub$DashboardSetting, dashboard_pt_setting)
micro_sub$DashboardPriority <- droplevels(micro_sub$DashboardPriority)

# Summarize Micro data and determine percentage of Micro labs within target
micro_rrl <- micro_sub %>%
  group_by(Test, Site, DashboardPriority, DashboardSetting, ReceiveResultTarget, CollectResultTarget, .drop = FALSE) %>%
  summarize(ResultedVolume = n(), ReceiveResultInTarget = sum(ReceiveResultInTarget), CollectResultInTarget = sum(CollectResultInTarget),
            ReceiveResultPercent = round(ReceiveResultInTarget/ResultedVolume, digits = 3), CollectResultPercent = round(CollectResultInTarget/ResultedVolume, digits = 3))

# Create a custom function to clean up summarized data
clean_summarized_data <- function(x) {
  ...
  # Format TAT percentages based on status definitions
  x <- x %>%  mutate(ReceiveResultPercent = cell_spec(ReceiveResultPercent, "html", color = ifelse(is.na(ReceiveResultPercent), "lightgray", ifelse(ReceiveResultPercent >= 0.95, "green", ifelse(ReceiveResultPercent >=0.8 &  ReceiveResultPercent < 0.95, "orange", "red")))))
...
chemistry2 <- clean_summarized_data(chemistry)
hematology2 <- clean_summarized_data(hematology)
micro2 <- clean_summarized_data(micro_rrl)


# Manually remove unused combinations
chemistry2 <- chemistry2[!(((chemistry2$Test == "Troponin" | chemistry2$Test == "Lactate WB") & (chemistry2$DashboardPriority != "All" | chemistry2$DashboardSetting == "Amb")) | (chemistry2$Test == "BUN" & chemistry2$DashboardPriority == "All")),]
```
...
### *Chemistry KPI*
<h4>Status Definitions: <span style = "color:red">Red:</span> <80%, 
<span style = "color:orange">Yellow:</span> >=80% & <90%,
<span style = "color:green">Green:</span> >=90%</h4>
```{r Chemistry Dashboard Tables, warning = FALSE, message = FALSE, echo = FALSE}
chemistry_dashboard <- melt(chemistry2, id.var = c("Test", "Site", "DashboardPriority", "TestAndPriority", "DashboardSetting", "ReceiveResultTarget", "CollectResultTarget"), measure.vars = c("ReceiveResultPercent", "CollectResultPercent"))

chemistry_summary <- dcast(chemistry_dashboard, Test + DashboardPriority + TestAndPriority + DashboardSetting +ReceiveResultTarget + CollectResultTarget ~ variable + Site, value.var = "value")
...
# Melt Hematology data into simpler data frame
hematology_dashboard <- melt(hematology2, id.var = c("Test", "Site", "DashboardPriority", "TestAndPriority", "DashboardSetting", "ReceiveResultTarget", "CollectResultTarget"), measure.vars = c("ReceiveResultPercent", "CollectResultPercent"))

# Cast formatted data
hematology_summary <- dcast(hematology_dashboard, Test + DashboardPriority + TestAndPriority + DashboardSetting +ReceiveResultTarget + CollectResultTarget ~ variable + Site, value.var = "value")
...
hem_table %>%
  kable(format = "html", escape = FALSE, align = "c", 
        col.names = c("Test & Priority", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL")) %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11) %>%
  add_header_above(c(" " = 1, "Receive to Result Within Target" = (ncol(hem_table)-1)/2, "Collect to Result Within Target" = (ncol(hem_table)-1)/2), background = c("white", "#00AEEF", "#221f72"), color = "white", line = FALSE, font_size = 13) %>%
  column_spec(column = c(1, 9, 17), border_right = "thin solid lightgray") %>%
  column_spec(column = 2:9, background = "#00AEEF", color = "white", include_thead = TRUE) %>%
  column_spec(column = 10:17, background = "#221f72", color = "white", include_thead = TRUE) %>%
  column_spec(column = 2:17, background = "inherit", color = "inherit") %>%
...
### *Microbiology RRL KPI*
<h4>Status Definitions: <span style = "color:red">Red:</span> <80%, 
<span style = "color:orange">Yellow:</span> >=80% & <90%,
<span style = "color:green">Green:</span> >=90%</h4>
```{r Microbiology Dashboard Tables, warning = FALSE, message = FALSE, echo = FALSE}
# Melt Micro data into simpler data frame
micro_rrl_tat_dashboard <- melt(micro2, id.var = c("Test", "Site", "DashboardPriority", "TestAndPriority", "DashboardSetting", "ReceiveResultTarget", "CollectResultTarget"), measure.vars = c("ReceiveResultPercent", "CollectResultPercent"))

# Cast formatted data
micro_rrl_tat_summary <- dcast(micro_rrl_tat_dashboard, Test + DashboardPriority + TestAndPriority + DashboardSetting +ReceiveResultTarget + CollectResultTarget ~ variable + Site, value.var = "value")
...
micro_table %>%
  kable(format = "html", escape = FALSE, align = "c",
        col.names = c("Test & Priority", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL")) %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11) %>%
  add_header_above(c(" " = 1, "Receive to Result Within Target" = (ncol(micro_table)-1)/2, "Collect to Result Within Target" = (ncol(micro_table)-1)/2), background = c("white", "#00AEEF", "#221f72"), color = "white", line = FALSE, font_size = 13) %>%
  column_spec(column = c(1, 9, 17), border_right = "thin solid lightgray") %>%
  column_spec(column = 2:9, background = "#00AEEF", color = "white", include_thead = TRUE) %>%
  column_spec(column = 10:17, background = "#221f72", color = "white", include_thead = TRUE) %>%
  column_spec(column = 2:17, background = "inherit", color = "inherit") %>%
...
micro_rrl_volume <- melt(micro2, id.var = c("Test", "Site", "DashboardPriority", "TestAndPriority", "DashboardSetting", "ReceiveResultTarget", "CollectResultTarget"), measure.vars = "ResultedVolume")

micro_rrl_volume_summary <- dcast(micro_rrl_volume, Test + DashboardPriority + TestAndPriority + DashboardSetting +ReceiveResultTarget + CollectResultTarget ~ variable + Site, value.var = "value") 

micro_rrl_volume_summary[c("ReceiveResultTarget", "CollectResultTarget")] <- "Resulted Volume"
original_length <- ncol(micro_rrl_volume_summary)

micro_rrl_volume_summary <- micro_rrl_volume_summary[ ,c(1:ncol(micro_rrl_volume_summary), (ncol(micro_rrl_volume_summary)-(6-1)):ncol(micro_rrl_volume_summary))]

micro_rrl_volume_summary <- micro_rrl_volume_summary[ , c(1:3, 5, 4, 7:12, 6, 4, 13:18)]
colnames(micro_rrl_volume_summary) <- colnames(micro_rrl_tat_summary)

micro_rrl_volume_summary[ , (original_length):ncol(micro_rrl_volume_summary)] <- " "

micro_rrl_comb <- rbind(micro_rrl_tat_summary, micro_rrl_volume_summary)

micro_rrl_comb2 <- micro_rrl_comb[order(micro_rrl_comb$Test, micro_rrl_comb$ReceiveResultTarget), ]
row.names(micro_rrl_comb2) <- 1:nrow(micro_rrl_comb2)

micro_table2 <- micro_rrl_comb2[ , 3:ncol(micro_rrl_comb2)]

micro_table2 %>%
  kable(format = "html", escape = FALSE, align = "c",
        col.names = c("Test & Priority", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL", "Target", "Setting", "MSH", "MSQ", "MSBI", "MSB", "MSW", "MSSL")) %>%
  kable_styling(bootstrap_options = "hover", position = "center", font_size = 11) %>%
  add_header_above(c(" " = 1, "Receive to Result Within Target" = (ncol(micro_table2)-1)/2, "Collect to Result Within Target" = (ncol(micro_table2)-1)/2), background = c("white", "#00AEEF", "#221f72"), color = "white", line = FALSE, font_size = 13) %>%
  column_spec(column = c(1, 9, 17), border_right = "thin solid lightgray") %>%
  column_spec(column = 2:9, background = "#00AEEF", color = "white", include_thead = TRUE) %>%
  column_spec(column = 10:17, background = "#221f72", color = "white", include_thead = TRUE) %>%
  column_spec(column = 2:17, background = "inherit", color = "inherit") %>%
...
<h4>Status Definitions: <span style = "color:red">Red:</span> >15%, 
<span style = "color:orange">Yellow:</span> <=15% & >5%,
<span style = "color:green">Green:</span> <=5%</h4>
...
```{r Conditional Block for Weekends, eval = include_not_wkday}
