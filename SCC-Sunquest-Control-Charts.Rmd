...
scale <- mean(vol_min/percent_min, vol_max/percent_max)
...
ggplot() +
  ...
  scale_y_continuous(limits = c(NA, 1), expand = c(0,0), sec.axis = sec_axis(~.*scale, name = "Volume of Labs"))
...
scale <- mean(min_vol/min_tat, max_vol/max_tat)
...
ggplot() +
  ...
  geom_rect(aes(xmin = start_date - 1, xmax = max(data_tat_plot$ResultDate) + 1, ymin = min(data_tat_plot$NotSafeThreshold), ymax = min(data_tat_plot$SafeThreshold), fill = "At Risk", alpha = "At Risk")) +
  geom_rect(aes(xmin = start_date - 1, xmax = max(data_tat_plot$ResultDate) + 1, ymin = min(data_tat_plot$SafeThreshold), ymax = Inf, fill = "Safe", alpha = "Safe")) +    
  ...
  scale_alpha_manual(name = "Status", values = c("Not Safe" = 0.15, "At Risk" = 0.15, "Safe" = 0.15), breaks=c("Not Safe","At Risk","Safe")) +
  scale_color_manual(name = "Setting", values = mshs_colors) +
  scale_size_manual(name = paste0(lab_priority, " Lab Volume"), values = 2, labels = "All Settings") + 
  scale_linetype_manual(name = paste0(lab_priority, " Lab Volume"), values = "dotted", labels = "All Settings") +
  guides(color = guide_legend(order = 1), shape = guide_legend(order = 1), fill = guide_legend(order = 3), alpha = guide_legend(order = 3), size = guide_legend(order = 2), linetype = guide_legend(order = 2)) +
  scale_y_continuous(limits = c(NA, 1), expand = c(0,0.01), sec.axis = sec_axis(~.*scale, name = "Volume of Labs"))
