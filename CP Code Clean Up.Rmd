  ...
  mutate(
    ...
    Incl = ifelse(
      ...
      (DashboardPriority %in% c("Routine") &
        ...
        # Remove priority stratification for rapid flu since all are treated
        # as stat
        (Test %in% c("Rapid Flu") & !(DashboardPriority %in% c("All"))) |
        # Remove priority stratification and ambulatory for c. diff labs
        (Test %in% c("C. diff") & (!(DashboardPriority %in% c("All")) |
                                       (DashboardSetting %in% c("Amb")))),
    "Excl", "Incl")) %>%
  filter(Incl == "Incl")
...
# lab_summary <- lab_summary %>%
#   mutate(ReceiveResultPercent = ifelse(!is.finite(ReceiveResultPercent),
#                                        NA, ReceiveResultPercent),
#          CollectResultPercent = ifelse(!is.finite(CollectResultPercent),
#                                        NA, CollectResultPercent))
...
```
```{r Left join test}
chem_comb_test <- test_priority_setting_df %>%
  filter(Division == "Chemistry")

test_left_join <- left_join(test_priority_setting_df, lab_summary,
                            by = c("Test" = "Test",
                                   "DashboardPriority" = "DashboardPriority",
                                   "DashboardSetting" = "DashboardSetting"))
