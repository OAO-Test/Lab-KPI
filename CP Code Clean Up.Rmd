...
```{r}
# Preprocess data
scc_sun_wday_list <-preprocess_cp(raw_scc = SCC_Weekday, raw_sun = SQ_Weekday)
...
if (is.null(SQ_Not_Weekday) & is.null(SCC_Not_Weekday)) {
  ...
  scc_sun_not_wday_list <- preprocess_cp(
...
```{r Summarize weekday/not weekday data for tables and repository}
...
```{r Create template dataframes for TAT and volume tables}
...
```{r New function for subsetting lab data and creating kables using repo structure}
summarize_cp_tat <- function(x, LabDivision) {
...
chem_list_test <- summarize_cp_tat(x = cp_wday_summary,
...
kable_cp_tat(x = chem_cast_test)

hem_list_test <- summarize_cp_tat(x = cp_wday_summary,
...
kable_cp_tat(x = hem_cast_test)

micro_list_test <- summarize_cp_tat(x = cp_wday_summary,
...
summarize_cp_vol <- function(x, LabDivision) {
...
kable_cp_vol(summarize_cp_vol(x = cp_wday_summary,
                   LabDivision = "Chemistry"))
...
```{r Existing code for missing collections and add on order volume}
...
```{r Custom functions for missing collections and add on orders}
...
# Custom function for add-on order volume
