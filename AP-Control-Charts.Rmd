...
historical_30_day_period_defects <-summarise(group_by(historical_30_day_period, Signed_out_date_only,Spec_group, Facility), No_cases_signed_out = sum(No_cases_signed_out),total_none_defects = sum(total_none_defects), total_defects = sum(total_defects), defective_rate = total_defects/No_cases_signed_out)
...
defective_rate_pchart <- function(historical_30_day_period_defects, test_name, site_name) {
  ...
  center_line <- mean(QC_Lines(data = historical_30_day_period_defects$defective_rate, n = historical_30_day_period_defects$No_cases_signed_out, method = "p")$pBar)
  
  # Create plot and standard settings
  ggplot(data = historical_30_day_period_defects[historical_30_day_period_defects$Spec_group == test_name & historical_30_day_period_defects$Facility == site_name, ], aes(x = Signed_out_date_only, y = defective_rate, n = No_cases_signed_out)) +
  ...
  geom_label(x = max(historical_30_day_period_defects$No_cases_signed_out)+1, y = center_line, vjust = 1.2, hjust = 1, label = paste("CL:", percent(center_line, accuracy = 0.1))) +
  # Specify graph labels and themes
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.position = "bottom", legend.box = "horizontal", 
          ...
          axis.text.x = element_text(angle = 30, hjust = 1)) +
  # Specify axis properties
  scale_x_date(limits = c(first_day_in_reporting_period -1, max(historical_30_day_period_defects$Signed_out_date_only)+1), breaks = seq(first_day_in_reporting_period, max(historical_30_day_period_defects$Signed_out_date_only), by = 3), date_minor_breaks = "1 day", date_labels = "%m/%d/%y", expand = c(0, 0, 0, 0)) +
  scale_y_continuous(labels = percent_format(accuracy = 1))
}

defective_rate_pchart(historical_30_day_period_defects, "GI", "MSH")
