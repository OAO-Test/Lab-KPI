...
#Today <- as.timeDate(as.Date("02/19/2020", format = "%m/%d/%Y"))

#Determine if yesterday was a holiday/weekend 
#get yesterday's DOW
Yesterday <- as.timeDate(format(Sys.Date()-1,"%m/%d/%Y"))
#Yesterday <- as.timeDate(as.Date("02/18/2020", format = "%m/%d/%Y"))
...
pre_processing_pp <- function(Raw_Data){
  if (is.null(Raw_Data) || nrow(Raw_Data)==0){
    ...
    Raw_Data_New_Cases_Signed_Stratified_sum <- summarise(group_by(Raw_Data_New,spec_group, Facility, Patient.Setting), No_Cases_Signed = n())    
    Raw_Data_New_Cases_Signed_Stratified <- dcast(Raw_Data_New_Cases_Signed_Stratified_sum, spec_group + Patient.Setting ~ Facility, value.var = "No_Cases_Signed" )
    Raw_Data_New_Cases_Signed_Stratified[is.na(Raw_Data_New_Cases_Signed_Stratified)] <- 0
        
    Raw_Data_New_Patient_Metric_sum <- summarise(group_by(Raw_Data_New,spec_group, Facility,Patient.Setting), Avg_Collection_to_Signed_out=format(ceiling(mean(Collection_to_signed_out, na.rm = TRUE))))
    
    Raw_Data_New_Patient_Metric <- dcast(Raw_Data_New_Patient_Metric_sum, spec_group + Patient.Setting ~ Facility, value.var = "Avg_Collection_to_Signed_out" )
    
    
    #Calculate % Receive to result TAT within target
    Raw_Data_New_Lab_Metric_sum <-summarise(group_by(Raw_Data_New,spec_group, Facility,Patient.Setting), Received_to_Signed_out_within_target = format(round(sum(Received_to_signed_out <= Received.to.signed.out.target..Days., na.rm = TRUE)/sum(Received_to_signed_out >= 0, na.rm = TRUE),2)))
    
    Raw_Data_New_Lab_Metric <- dcast(Raw_Data_New_Lab_Metric_sum, spec_group + Patient.Setting ~ Facility, value.var = "Received_to_Signed_out_within_target" )
...
```{r Add the current summary to the historical repo for cytology/pathology, echo = FALSE, warning = FALSE, message = FALSE}

#1. import the historical repo
Historical_Repo <- read.csv(paste0(user_directory,"/AP & Cytology Historical Repo/" ,"Historical_Repo_Surgical_Pathology","_",Yesterday,".csv"), stringsAsFactors = FALSE)
...
```{r create a custom function for 30 days volume lookback,warning = FALSE, message = FALSE, echo = FALSE}
#exclude weekends
Historical_Repo_Updated_weekdays <- Historical_Repo_Updated[which(Historical_Repo_Updated$Signed_out_day_only != "Sunday" & Historical_Repo_Updated$Signed_out_day_only != "Saturday"),]

#determine the 30 business that will be reported
unique_dates <- unique(Historical_Repo_Updated_weekdays$Signed_out_date_only)
...
historical_30_day_period <- Historical_Repo_Updated_weekdays[which(Historical_Repo_Updated_weekdays$Signed_out_date_only>=first_day_in_reporting_period),]
...
historical_30_day_period_breast_MSH <- historical_30_day_period_sum[historical_30_day_period_sum$Spec_group == "Breast"& (historical_30_day_period_sum$Patient_setting == "Amb" | historical_30_day_period_sum$Patient_setting == "IP") & historical_30_day_period_sum$Facility == "MSH",]

historical_30_day_period_vol_breast_MSH <- historical_30_day_period_sum_vol[historical_30_day_period_sum_vol$Spec_group == "Breast" & historical_30_day_period_sum_vol$Facility == "MSH",]
...
ggplot()+
  ...
  geom_line(data = historical_30_day_period_vol_breast_MSH, aes(x = Signed_out_date_only, y = No_cases_signed_out/max(No_cases_signed_out)), linetype = "dashed")+ 
  geom_point(data = historical_30_day_period_vol_breast_MSH, aes(x = Signed_out_date_only, y = No_cases_signed_out/max(No_cases_signed_out)), size = 2, shape = 4)+
  geom_point(data = historical_30_day_period_breast_MSH, aes_string(x = "Signed_out_date_only", y = "Lab_metric_within_target", color = "Patient_setting", shape = "Patient_setting"), size = 3) +
  geom_line(data = historical_30_day_period_breast_MSH, aes_string(x = "Signed_out_date_only", y = "Lab_metric_within_target", color = "Patient_setting"), linetype = "dashed") +
  labs(title = "All Breast Specimen for MSH", x = "Date",  y = "Percentage of Labs", color = "Patient Setting", shape = "Patient Setting") +
  scale_x_date(name = "Date", labels = date_format("%m/%d/%Y"), limits = c(first_day_in_reporting_period - 1,last_day_in_reporting_period + 1), date_breaks = "3 days", date_minor_breaks = "1 day", expand = c(0, 0))+
  theme(plot.title = element_text(hjust = 0.5), legend.position = "right", legend.box = "vertical", legend.title = element_text(size = 10), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 45, hjust = 1), panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  scale_y_continuous(name = "Percentage of Labs",limits = c(0,1), labels = percent, sec.axis = sec_axis(~.*max(historical_30_day_period_vol_breast_MSH$No_cases_signed_out), name = "Labs Volume")) +
  ...
  scale_color_manual(name = "Patient Setting", values = mshs_colors)
...
, Surgical_Pathology_Stratified_Vol_Weekday,pathology_volume_column_order)
Surgical_Pathology_Stratified_Vol_Not_Weekday_New2 <- Table_Merging_Volume(Table_Template_Patho_Vol
, Surgical_Pathology_Stratified_Vol_Not_Weekday,pathology_volume_column_order)
...
Backlog_Accessioned_Table_New2 %>% 
  ...
  row_spec(row = 0, font_size = 13)%>%
  add_header_above(c(" "= 2, "Elapsed Turn Around Time for Backloged Cases From the Received Time"= 4), background = c("white", "#221F72"), color= "white", font_size = 13)%>%
  collapse_rows(columns = 1)
```
