...
```{r Determine date, warning = TRUE, message = FALSE, echo = FALSE}
...
# Yesterday <- as.timeDate(as.Date("11/24/2019", format = "%m/%d/%Y"))
...
```{r Import all data, warning = TRUE, message = FALSE, echo = FALSE}
...
if(((Holiday_Det) & (Yesterday_Day =="Mon"))|((Yesterday_Day =="Sun") & (isHoliday(Yesterday-(86400*2))))){ # Scenario 1: Mon Holiday or Friday Holiday (Need to select 4 files)
  # Import SCC data
  SCC_Holiday_Monday_or_Friday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on a Holiday"), sheet = 1, col_names = TRUE)
  SCC_Sunday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on Sunday"), sheet = 1, col_names = TRUE)
  SCC_Saturday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on Saturday"), sheet = 1, col_names = TRUE)
  ...
  SCC_Weekday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on a Weekday"), sheet = 1, col_names = TRUE)
  # 
  # Import Sunquest data
  SQ_Holiday_Monday_or_Friday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on a Holiday"), sheet = 1, col_names = TRUE)
  SQ_Sunday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on Sunday"), sheet = 1, col_names = TRUE)
  SQ_Saturday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on a Saturday"), sheet = 1, col_names = TRUE)
  #Merge the weekend data with the holiday data in one data frame
  SQ_Not_Weekday <- rbind(SQ_Holiday_Monday_or_Friday, SQ_Sunday, SQ_Saturday)
  SQ_Weekday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on a Weekday"), sheet = 1, col_names = TRUE)
  #
  # Import Powerpath data
  PP_Holiday_Monday_or_Friday <- read_excel(choose.files(caption = "Select PowerPath Report for Labs Resulted on a Holiday"), skip = 1, 1)
  PP_Holiday_Monday_or_Friday  <- PP_Holiday_Monday_or_Friday[-nrow(PP_Holiday_Monday_or_Friday),]
  PP_Sunday <- read_excel(choose.files(caption = "Select PowerPath Report for Labs Resulted on Sunday"), skip = 1, 1)
  PP_Sunday <- PP_Sunday[-nrow(PP_Sunday),]
  PP_Saturday <- read_excel(choose.files(caption = "Select PowerPath Report for Labs Resulted on Saturday"), skip = 1, 1)
  ...
  PP_Weekday <- read_excel(choose.files(caption = "Select PowerPath Report for Labs Resulted on a Weekday"), skip = 1, 1)
  ...
  # Import Sunquest data
  SQ_Sunday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on Sunday"), sheet = 1, col_names = TRUE)
  SQ_Saturday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on Saturday"), sheet = 1, col_names = TRUE)
  #Merge the weekend data with the holiday data in one data frame
  ...
  #
  # Import SCC data
  SCC_Holiday_Weekday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on a Holiday"), sheet = 1, col_names = TRUE)
  SCC_Not_Weekday <- SCC_Holiday_Weekday
  SCC_Weekday <- read_excel(choose.files(caption = "Select SCC Report for Labs Resulted on a Weekday"), sheet = 1, col_names = TRUE)
  #
  # Import Sunquest data
  SQ_Holiday_Weekday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on a Holiday"), sheet = 1, col_names = TRUE)
  SQ_Not_Weekday <- SQ_Holiday_Weekday
  SQ_Weekday <- read_excel(choose.files(caption = "Select Sunquest Report for Labs Resulted on a Weekday"), sheet = 1, col_names = TRUE)
  #
  # Import Powerpath data
  PP_Holiday_Weekday <- read_excel(choose.files(caption = "Select PowerPath Report for Labs Resulted on a Holiday"), skip = 1, 1)
...
```{r Reference Data, warning = TRUE, message = FALSE, echo = FALSE}
# CP and Micro --------------------------------
# Import analysis reference data starting with test codes for SCC and Sunquest
reference_file <- "J:\\deans\\Presidents\\HSPI-PM\\Operations Analytics and Optimization\\Projects\\Service Lines\\Lab KPI\\Data\\Code Reference\\Analysis Reference 2020-01-22.xlsx"
...
```{r Custom Function for preprocessing raw SCC and Sunquest data, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Data Preprocessing and Merging for SCC and Sunquest data, message = FALSE, warning = TRUE, echo = FALSE}
...
```{r Custom function for preprocessing raw Powerpath data and calling function, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Cytology backlog preprocessing and analysis, echo=FALSE, warning=TRUE, message=FALSE}
...
```{r Custom Functions for subsetting and summarizing data for SCC and Sunquest dashboard tables, warning = TRUE, message = FALSE, echo = FALSE}
...
Table_Merging_Patho_Volume <- function(Surgical_Pathology_Vol_Table){
  if (is.null(Surgical_Pathology_Vol_Table)){
    Surgical_Pathology_Vol_Table_New <- NULL
    Surgical_Pathology_Vol_Table_New2 <- NULL
  } else {
    #first step is merging the table template with the cytology table and this will include all of the missing columns.

    Surgical_Pathology_Vol_Table_New <- merge(x = Table_Template_Patho_Vol, y= Surgical_Pathology_Vol_Table, all.y=TRUE)
    
    #second step is merging the table with all of the columns with only the first two columns of the template to include all the missing rows
    Surgical_Pathology_Vol_Table_New2 <- merge(x = Table_Template_Patho_Vol[c(1,2)], y= Surgical_Pathology_Vol_Table_New, all.x = TRUE, by = c("spec_group", "Patient.Setting"))
    rows_order_Patho_Vol <- factor(rownames(Surgical_Pathology_Vol_Table_New2),levels = c(2,1,4,3))
    Surgical_Pathology_Vol_Table_New2 <- Surgical_Pathology_Vol_Table_New2[order(rows_order_Patho_Vol), columns_order_V3]
    row.names(Surgical_Pathology_Vol_Table_New2) <- NULL
  }
  return(Surgical_Pathology_Vol_Table_New2)
}


Surgical_Pathology_Stratified_Vol_Weekday_New2 <- Table_Merging_Patho_Volume(Surgical_Pathology_Stratified_Vol_Weekday)
Surgical_Pathology_Stratified_Vol_Not_Weekday_New2 <- Table_Merging_Patho_Volume(Surgical_Pathology_Stratified_Vol_Not_Weekday)
...
Table_Formatting_Volume <- function (Volume_Table){
  if (is.null(Volume_Table)){
    ...
    kable_styling(bootstrap_options = "hover", full_width = FALSE, position = "center", row_label_position = "c", font_size = 11) %>%
    column_spec(3:11, background = "gray",color ="white", include_thead = TRUE)%>%
...
### Surgical Pathology
#### *Surgical Pathology KPI (Labs Resulted on `r wday_result_date`)*
<h5>Status Definitions: <span style = "color:red">Red:</span> <80%, 
...
### Cytology
#### *Cytology KPI (Labs Resulted on `r wday_result_date`)*
<h5>Status Definitions: <span style = "color:red">Red:</span> <80%, 
<span style = "color:orange">Yellow:</span> >=80% & <90%,
<span style = "color:green">Green:</span> >=90%</h5>
...
```{r Custom function for 24 hour volume lookbacks for Chemistry, Hematology, and Micro RRL, echo = FALSE, warning = FALSE, message = FALSE}
...
lab_volume_summarize <- function(x, LabDivision) {
  ...
  lab_vol_summary$TestPriority <- paste(lab_vol_summary$Test, "-", lab_vol_summary$DashboardPriority)
