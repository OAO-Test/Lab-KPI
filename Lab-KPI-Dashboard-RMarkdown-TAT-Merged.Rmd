...
#Patho
Table_Template_Patho <- data.frame(matrix(ncol=19, nrow = 4)) 
colnames(Table_Template_Patho) <- c("spec_group","Patient.Setting","No_Cases_Signed","MSH.x", "BIMC.x","MSQ.x", "MSS.x","PACC.x","R.x","SL.x", "KH.x","BIMC.y", "MSH.y", "MSQ.y", "MSS.y","PACC.y","R.y","SL.y", "KH.y")
Table_Template_Patho[1] <- c('Breast','Breast','GI', 'GI')
Table_Template_Patho[2] <- c('IP', 'Amb')

Table_Template_Patho_Vol <- data.frame(matrix(ncol=10, nrow = 4)) 
colnames(Table_Template_Patho_Vol) <- c("spec_group","Patient.Setting","BIMC", "MSH", "MSQ", "MSS","PACC","R","SL", "KH")
...
Table_Merging_Patho <- function(Surgical_Pathology_Table){
  if (is.null(Surgical_Pathology_Table)){
    ...
    Surgical_Pathology_Table_New2 <- Surgical_Pathology_Table_New2[order(rows_order_Patho), c("spec_group","Patient.Setting","No_Cases_Signed","MSH.x","MSS.x","MSQ.x","BIMC.x","PACC.x","KH.x","R.x","SL.x", "MSH.y", "MSS.y","MSQ.y","BIMC.y","PACC.y","KH.y","R.y","SL.y")]
  }
...
pathology_volume_column_order <- c("spec_group","Patient.Setting","MSH", "MSS","MSQ","BIMC","PACC","KH","R","SL")  
...
Conditional_Formatting_Patho <- function(Table_New2){
  if (is.null(Table_New2)){
    ...
    Table_New3 <- melt(Table_New2, id =c("spec_group","Patient.Setting","No_Cases_Signed","MSH.y","MSS.y","MSQ.y","BIMC.y","PACC.y","KH.y","R.y","SL.y"))
    ...
    Table_New3 <- dcast(Table_New3,spec_group + Patient.Setting + No_Cases_Signed + BIMC.y + MSH.y + MSQ.y + MSS.y + PACC.y + R.y + SL.y + KH.y ~ variable )
    
    Table_New4 <- melt(Table_New3, id =c("spec_group","Patient.Setting","No_Cases_Signed","MSH.x","MSS.x","MSQ.x","BIMC.x","PACC.x","KH.x","R.x","SL.x"))
    
    
    Table_New4 <- Table_New4 %>%
      mutate(value = ifelse(is.na(value), cell_spec(value, "html", color = "lightgray"), cell_spec(value, "html", color = "black")))
    
    Table_New4 <- dcast(Table_New4,spec_group + Patient.Setting + No_Cases_Signed + BIMC.x + MSH.x + MSQ.x + MSS.x  + PACC.x + R.x + SL.x + KH.x ~ variable )
    
    Table_New5 <- merge(x=Table_New4, y=TAT_Targets[1:3])
    rows_order <- factor(rownames(Table_New5),levels = c(2,1,4,3))
    Table_New5 <- Table_New5[order(rows_order), c("spec_group","Received.to.signed.out.target..Days.","Patient.Setting","No_Cases_Signed","MSH.x", "MSS.x","MSQ.x","BIMC.x","PACC.x","KH.x","R.x","SL.x","MSH.y", "MSS.y","MSQ.y","BIMC.y","PACC.y","KH.y","R.y","SL.y")]
    Table_New5$Received.to.signed.out.target..Days.<- paste("<= ", Table_New5$Received.to.signed.out.target..Days., "days")
    row.names(Table_New5) <- NULL
    Table_New5$No_Cases_Signed[is.na(Table_New5$No_Cases_Signed)] <- 0
  }
...
pathology_standardized_column_names <-  c("Case Type","Target","Setting","No. Cases Signed Out","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL")
...
Table_Formatting <- function (Table_New3,column_names){
  if (is.null(Table_New3)){
    ...
    add_header_above(c(" "= 1, "Receive to Result within Target (Business Days)"= 11, "Average Collection to Result TAT (Calendar Days)"=8), background = c("white", "#00B9F2", "#221F72"), color= "white", font_size = 13)%>%
    column_spec(2:12, background = "#00B9F2",color ="white", include_thead = TRUE)%>%
    column_spec(13:20, background = "#221F72", color = "white", include_thead = TRUE)%>%
    column_spec(2:20, background = "inherit", color = "inherit")%>%
    row_spec(row = 0, font_size = 13)%>%
...
pathology_volume_column_names <-  c("Case Type","Setting","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL")
...
Table_Formatting_Volume <- function (Volume_Table, column_names){
  if (is.null(Volume_Table)){
    ...
    kable(escape = F, align = "c", col.names = column_names) %>%
    kable_styling(bootstrap_options = "hover", full_width = FALSE, position = "center", row_label_position = "c", font_size = 11) %>%
    column_spec(3:10, background = "#00B9F2",color ="white", include_thead = TRUE)%>%
    column_spec(3:10, background = "inherit", color = "inherit")%>%
    row_spec(row = 0, font_size = 13)%>%
...
Table_Formatting(Cytology_Table_Weekday_New3, cytology_standardized_column_names)
...
### Clinical Labs, Blood Bank & Anatomic Pathology Front End Operational Indicators
#### _Clinical Labs, Blood Bank & Anatomic Pathology Front End Ops Indicator Table Placeholder_
### Centralized Anatomic Pathology Operational Indicators
#### _Centralized Anatomic Pathology Ops Indicator Table Placeholder_
### Quality Indicators
...
Table_Formatting(Surgical_Pathology_Table_Not_Weekday_New3,pathology_standardized_column_names)
...
# asis_output('<h5>Status Definitions: <span style = "color:red">Red:</span> <80%, <span style = "color:orange">Yellow:</span> >=80% & <90%, <span style = "color:green">Green:</span> >=90%</h5>')
Table_Formatting(Cytology_Table_Not_Weekday_New3,cytology_standardized_column_names)
Table_Formatting_V2(Cytology_Table_Not_Weekday_New3_V2)
...
### Surgical Pathology   
#### *Surgical Pathology Signed Out Cases Volume (As of `r wkend_holiday_result_date`)*
```{r Surgical Pathology Weekend/Holiday: volume lookback, echo=FALSE, warning=FALSE, message=FALSE}
Table_Formatting_Volume(Surgical_Pathology_Stratified_Vol_Not_Weekday_New2, pathology_volume_column_names)
```

### Cytology   
#### *Cytology Signed Out Cases Volume (As of `r wkend_holiday_result_date`)*
```{r Cytology Weekend/Holiday: volume lookback, echo=FALSE, warning=FALSE, message=FALSE}
Table_Formatting_Volume(Cytology_Stratified_Vol_Not_Weekday_New2, cytology_volume_column_names)
```
