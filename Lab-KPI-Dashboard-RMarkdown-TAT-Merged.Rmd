...
pre_processing_pp <- function(Raw_Data){
  if (is.null(Raw_Data) || nrow(Raw_Data)==0){
    ...
    Summarized_Table <- summarise(group_by(Raw_Data_New,spec_group, Facility,Patient.Setting), No_Cases_Signed = n(), Lab_Metric_TAT_Avg =mean(Received_to_signed_out, na.rm = TRUE), Lab_Metric_TAT_med =median(Received_to_signed_out, na.rm = TRUE), Lab_Metric_TAT_sd =sd(Received_to_signed_out, na.rm = TRUE), Received_to_Signed_out_within_target = format(round(sum(Received_to_signed_out <= Received.to.signed.out.target..Days., na.rm = TRUE)/sum(Received_to_signed_out >= 0, na.rm = TRUE),2)), Avg_Collection_to_Signed_out=format(ceiling(mean(Collection_to_signed_out, na.rm = TRUE))), Patient_Metric_TAT_med =median(Collection_to_signed_out, na.rm = TRUE), Patient_Metric_TAT_sd =sd(Collection_to_signed_out, na.rm = TRUE))
...
```{r Create historical repo for cytology/pathology, echo = FALSE, warning = FALSE, message = FALSE}
