...
historical_30_day_period <- Historical_Repo_Updated_weekdays[which(Historical_Repo_Updated_weekdays$Signed_out_date_only>=last_day_in_reporting_period),]

#summarized the data to plot it correctly
historical_30_day_period_sum <-summarise(group_by(historical_30_day_period, Signed_out_date_only,Spec_group, Facility,Patient_setting), Lab_metric_within_target = format(round(mean(as.numeric(Lab_metric_within_target), na.rm = TRUE), 2)), Patient_metric_avg = format(round(mean(as.numeric(Patient_metric_avg), na.rm = TRUE),0)))
...
ggplot()+
  ...
  geom_point(data = historical_30_day_period_breast_MSH, aes(x = Signed_out_date_only, y = Lab_metric_within_target, group= Patient_setting, color = Patient_setting, shape = Patient_setting)) +
  geom_line(data = historical_30_day_period_breast_MSH, aes(x = Signed_out_date_only, y = Lab_metric_within_target, group= Patient_setting, color = Patient_setting, shape = Patient_setting)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1), panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  scale_y_continuous(name = "Percentage of Labs",limits = c(0,1), labels = percent) +
