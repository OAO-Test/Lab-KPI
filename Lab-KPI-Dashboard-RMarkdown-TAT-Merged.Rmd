...
if(((Holiday_Det) & (Yesterday_Day =="Mon"))|((Yesterday_Day =="Sun") & (isHoliday(Yesterday-(86400*2))))){ # Scenario 1: Mon Holiday or Friday Holiday (Need to select 4 files)
  # Import SCC data
  SCC_Holiday_Monday_or_Friday <- read_excel(choose.files(caption = "Select SCC Mon/Fri Holiday Report"), sheet = 1, col_names = TRUE)
  ...
  SQ_Holiday_Monday_or_Friday <- read_excel(choose.files(caption = "Select Sunquest Mon/Fri Holiday Report"), sheet = 1, col_names = TRUE)
...
Cytology_Table_Weekday <- pre_processing_pp(Cytology_Weekday_Raw)
Cytology_Table_Not_Weekday <- pre_processing_pp(Cytology_Not_Weekday_Raw)
Surgical_Pathology_Table_Weekday <- pre_processing_pp(Surgical_Pathology_Weekday)
Surgical_Pathology_Table_Not_Weekday <- pre_processing_pp(Surgical_Pathology_Not_Weekday)
```
...
```{r Subset and format SCC and Sunquest data for each lab division weekday dashboard, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Custom functions for formatting Cytology dashboard tables, echo = FALSE, warning = TRUE, message = FALSE}
...
```{r Custom function for Anatomic Pathology kable formatting, echo=FALSE, warning = TRUE, message = FALSE}
Table_Formatting <- function (Table_New3){
  if (is.null(Table_New3)){
    ...
    Table_New3%>%
    select(everything()) %>%
    kable(escape = F, align = "c", col.names = c("Case Type","Target","Setting","No. Cases Signed","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE","MSH", "MSSM","MSQ","MSBI","PACC","MSB","MSW","MSSL", "NYEE")) %>%
    ...
    add_header_above(c(" "= 4, "Receive to Result within Target (Business Days)"= 9, "Average Collection to Result TAT (Calendar Days)"=9), background = c("white", "#00B9F2", "#221F72"), color= "white")
  }
}
```
...
```{r Chemistry dashboard table, warning = TRUE, message = FALSE, echo = FALSE}
...
<span style = "color:orange">Yellow:</span> >=80% & <95%,
<span style = "color:green">Green:</span> >=95%</h5>
```{r Hematology dashboard table, warning = TRUE, message = FALSE, echo = FALSE}
chem_hem_micro_kable(hem_table)
```
<h6>*TAT Analysis excludes add on orders, labs with missing timestamps, labs with negative TAT, and labs not from above settings.*</h6>

```{r Microbiology RRL volume and TAT table creation, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Create Micro RRL TAT and Volume Table, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Missing Collection Times Tables, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Surgical Pathology Efficiency Indicator Weekday, echo=FALSE, warning=TRUE, message=FALSE}
...
<span style = "color:orange">Yellow:</span> >=80% & <90%,
<span style = "color:green">Green:</span> >=90%</h5>
```{r Cytology Efficiency Indicators Weekday, echo=FALSE, warning=TRUE, message=FALSE}
...
```{r Custom function for 24 hour volume lookbacks for Chemistry, Hematology, and Micro RRL, echo = FALSE, warning = TRUE, message = FALSE}
...
```{r Chemistry: 24 hour volume lookback, echo = FALSE, warning = TRUE, message = FALSE}
...
```{r Hematology: 24 hour volume lookback, echo = FALSE, warning = TRUE, message = FALSE}
...
```{r Cytology Backlog and Accessioned, echo=FALSE, warning=TRUE, message=FALSE}
...
```{r Conditional Block for Weekends, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Chemistry dashboard table for weekends and holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
...
```{r Hematology dashboard table for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
...
```{r Microbiology RRL volume and TAT table creation for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
...
```{r Microbiology RRL dashboard table for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
...
```{r Missing Collection Times Tables for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
...
```{r Surgical Pathology efficiency indicators for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
...
```{r Cytology efficiency indicators for weekends/holidays, eval = include_not_wday, warning = TRUE, message = FALSE, echo = FALSE}
asis_output('<h3>   </h3>')
