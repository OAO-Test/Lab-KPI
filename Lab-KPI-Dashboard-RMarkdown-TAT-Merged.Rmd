...
if (is.null(scc_sun_not_wday_master) == FALSE) {
  scc_sun_all_days_master <- rbind(scc_sun_wday_master, scc_sun_not_wday_master)
} else {
  scc_sun_all_days_master <- scc_sun_wday_master
}
...
scc_sun_all_days_subset <- scc_sun_all_days_master %>%
  group_by(Site, ResultDate, Test, Division, 
           ...
           ReceiveResultTarget, CollectResultTarget) %>%
  summarize(TotalResulted = n(), TotalResultedTAT = sum(TATInclude), TotalReceiveResultInTarget = sum(ReceiveResultInTarget), TotalCollectResultInTarget = sum(CollectResultInTarget), TotalAddOnOrder = sum(AddOnMaster == "AddOn"), TotalMissingCollections = sum(MissingCollect))


test_results <- scc_sun_all_days_subset %>%
  group_by(Site, Test, DashboardSetting, DashboardPriority) %>%
