...
historical_repo_form <- read_excel(
  paste0(user_directory, "/Lab KPI Form/Lab KPI Hsitorical Repo/",
         "Historical_Repo_Form", "_", yesterday, ".xlsx"))

#make sure the data types for both dataframes are the same
common <-
  names(historical_repo_form)[names(historical_repo_form) 
                              %in% names(kpi_form_today)]

historical_repo_form[common] <- lapply(common, function(x) {
  match.fun(paste0("as.",
                   class(kpi_form_today[[x]])))(historical_repo_form[[x]])
})
